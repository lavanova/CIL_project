    ...
    *********************************************************
    ...
    for epoch in range(args.epoch):
        ...
        for idx in range(n_batch):
            ...
            users, pos_items, pos_items_rating = data_generator.sample()
        ...
        #users_to_test = list(data_generator.test_set.keys())
        #ret = test(sess, model, users_to_test, drop_flag=True)
        users_test, pos_items_test, pos_items_rating_test = data_generator.sample_test()
        test_loss = 0
        test_rmse = 0
        test_mf_loss = 0
        for loop in range(users_test.shape[0]):
            i_rate_batch, my_mf_loss, my_loss = sess.run([model.batch_ratings, model.mf_loss, model.loss], {model.users: users_test[loop, :],
                                                          model.pos_items: pos_items_test[loop, :],
                                                          model.pos_items_rating: pos_items_rating_test[loop, :],
                                                          model.node_dropout: [0.]*len(eval(args.layer_size)),
                                                          model.mess_dropout: [0.]*len(eval(args.layer_size))})

            test_loss += my_loss / users_test.shape[0]
            test_mf_loss += my_mf_loss / user_test.shape[0]
        test_rmse = np.sqrt(test_mf_loss)
        ...
        """
        if args.verbose > 0:
            perf_str = 'Epoch %d [%.1fs + %.1fs]: train==[%.5f=%.5f + %.5f + %.5f], test_loss=[%.5f], ' \
                       'test_mf_loss=[%.5f], test_rmse=[%.5f]' % \
                       (epoch, t2 - t1, t3 - t2, loss, mf_loss, emb_loss, reg_loss, test_loss, test_mf_loss,
                        test_rmse)
            print(perf_str)      
        """
